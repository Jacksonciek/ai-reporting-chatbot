...
const shouldRenameRoom = (name?: string) => {
  ...
  return normalized === DEFAULT_ROOM_LABEL.toLowerCase() || normalized === 'percakapan baru';
};
...
export default function ChatbotExperience({
  ...
  return (
    ...
    >
        ...
        {isLoading ? (
          ...
          >
            <div className="flex flex-col items-center space-y-4 text-slate-200">
              <motion.div
                className="flex items-center justify-center w-16 h-16 rounded-full bg-slate-900/50 border border-slate-700/50 shadow-lg shadow-blue-900/30"
                animate={{ rotate: 360 }}
      ...
      <Link
        href="/dashboard"
        className={`fixed bottom-6 left-6 z-30 flex items-center space-x-2 rounded-full bg-gray-900/80 border border-gray-700/60 text-gray-100 shadow-lg hover:bg-gray-800 transition-all duration-200
          ${sidebarExpanded ? "px-4 py-2" : "px-3 py-3"}`}
        title="Dashboard"
      >
